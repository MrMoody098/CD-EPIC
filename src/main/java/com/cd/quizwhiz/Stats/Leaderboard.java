...
public class Leaderboard {
        ...
        Files.walk(Paths.get(dir))
                ...
                .forEach(path -> {
                    ...
                    try (BufferedReader reader = new BufferedReader(new FileReader(path.toFile()))) {
                        ...
                        //to ensure if players currentScore is their new highscore not to show duplicate scores
                        //if the max score read in the userfile is not the same as the current users score
    ...
    /**
     * Returns a 2D array of usernames, their top scores, and (currentUsername with currentScore)+(currentUsername2 with
     * currentScore2) from user data files in a directory.
     *
     * @param currentUsername The username of the current user.
     * @param currentScore The score of the current user.
     * @return A 2D array (String[][]) where each row contains a username and their top score in descending order of
     * score.
     * The row containing currentUsername will have currentScore as its second column.
     * @throws IOException If an I/O error occurs when opening or reading a file.
     */
    public static String[][] getLeaderboard(String currentUsername, int currentScore,String currentUsername2,int currentScore2) throws IOException {
        ...
        Files.walk(Paths.get(dir))
                ...
                .forEach(path -> {
                    ...
                    try (BufferedReader reader = new BufferedReader(new FileReader(path.toFile()))) {
                        ...
                        //to ensure if player one's or player two's current score is their new high score not to
                        // show duplicate scores
                        if (((maxScore != (currentScore) && maxScore != (currentScore2)) && (username != currentUsername))
