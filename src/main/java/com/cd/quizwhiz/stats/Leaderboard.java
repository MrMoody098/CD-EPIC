...
Use Meaningful Variable Names:
    dir changed to userFolder
    Instead of maxScore, userMaxScore for better clarity.

Improve Variable Initialization:
    Initialize the leaderboardList with an initial capacity if possible, as i know the size beforehand.
    This can improve performance, especially when dealing with a large number of users.

Improve User Score Comparison:
    Instead of comparing usernames (username != currentUsername), !username.equals(currentUsername).
    This ensures a proper string comparison.
    ...
    */
        ...
        public static String[][] getLeaderboard() throws IOException {
            ...
            String userFolder = Auth.USER_FOLDER;

            // Initialize an ArrayList to store the leaderboard data
            List<String[]> leaderboardList = new ArrayList<>(50); // Initial capacity

            // Walk through the directory and process each file
            Files.walk(Paths.get(userFolder)).filter(Files::isRegularFile).forEach(path -> {
                ...
                try (BufferedReader reader = new BufferedReader(new FileReader(path.toFile()))) {
                    ...
                    while ((line = reader.readLine()) != null) {
                        userMaxScore = Math.max(userMaxScore, Integer.parseInt(line));
                    }

                    // Add the username and their top score to the leaderboardList
                    leaderboardList.add(new String[]{username, String.valueOf(userMaxScore)});
                } catch (IOException e) {
            ...
            return leaderboardList.toArray(new String[0][0]);
        }
